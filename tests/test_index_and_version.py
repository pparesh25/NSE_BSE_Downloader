...
def test_version_history_drives_update_notification():
    ...
    notes = VERSION_HISTORY["1.2.0"]
    assert notes["release_date"] == "2026-09-15"
    assert any("audit" in item.lower() for item in notes["features"])
    assert any("turnover" in item.lower() for item in notes["features"])
    assert any("delivery" in item.lower() for item in notes["bug_fixes"])
    ...
    assert parsed["latest_version"] == "1.2.0"
    assert any(
        "audit" in item.lower()
...
def test_version_metadata_remains_readable_by_v1_0_1_clients():
    ...
    assert version is not None and version.group(1) == "1.2.0"
